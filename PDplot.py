...
#move it on the main folder 
#option for points and contour plots

#import data

#List of parameters:  "sign_parameter", "generic_quark_phase_a1", "generic_quark_phase_a2", "model4_mR", "model4_r1", "model4_r2", "model4_cnu", "model4_ce"
    
#List of Predictions: "theta13","theta12", theta23, delta, maiorana_phase_a1, maiorana_phase_a2, m1, mbetabeta, msq12, msq13, ye, ymu, ytau, MN1, MN2, MN3

def load_data():
    OBS = np.loadtxt('%s/GUTFITpredictions.txt'%args[0])
    PAR = np.loadtxt('%s/GUTFITpara.txt'%args[0])
    L = np.loadtxt('%s/GUTFITlikelihood.txt'%args[0])
    return OBS, PAR, L
    
    

#points really simple 
...
def massContour(ax, xArray, y4DArray, col, xlab, ylab=''):
    ...
    if y4DArray.shape[1]==3:
        ...
        ax.fill_between(xArray,y4DArray[:,0], y4DArray[:,1], color=col, alpha=0.15)
        ax.fill_between(xArray,y4DArray[:,2], y4DArray[:,3], color=col, alpha=0.05)
    else:
        return
...
if __name__ == "__main__":
    ...
    #load parameter and initialize the data
    ParNames = ["sign_parameter", "generic_quark_phase_a1", "generic_quark_phase_a2", "model4_mR", "model4_r1", "model4_r2", "model4_cnu", "model4_ce"]
    ObsNames = ["theta13","theta12", "theta23", "delta", "maiorana_phase_a1", "maiorana_phase_a2", "m1", "mbetabeta", "msq12", "msq13", "ye", "ymu", "ytau", "MN1", "MN2", "MN3"]
    OBS, PAR, L = load_data()
    xN = 4 
    yN = 5 
    xPAR = True
    ...
    plot = ax.scatter(x,y,c =z,s = 10, cmap = plt.cm.viridis, norm = normalize, marker= "o")
    
    
    #labelling later
    ax.set(xlabel ="MN1", ylabel= r"$MN3$")
    ...
    if nubetabeta:
        ...
        #use inverted with inverted ordering 
        ...
        massContour(ax, ml,mbbN,'r',r'$m_{1}$ (meV)', r'$m_{\beta \beta}$ (meV)')
        #addSensitivity(ax[2,1], 36, 156, .1, color = "teal", experiment_name="KamLAND-Zen Upper Limit")
    
       # massContour(ax[0,1], ml,mbbN,'g','$m_{1}$ (meV)', '$<m>_{ee}$ (meV)')
       # addSensitivity(ax[0,1], 36, 156, .1, color = "teal", experiment_name="KamLAND-Zen Upper Limit")
    
        ax.set_xlim(XaxisMin,XaxisMax)
        ax.set_ylim(YaxisMin,YaxisMax)
    
        #ax[0,1].set_xlim(XaxisMin,XaxisMax)
        #ax[0,1].set_ylim(YaxisMin,YaxisMax)
...
#contour plot, implement all possibilities with options so easier for future
