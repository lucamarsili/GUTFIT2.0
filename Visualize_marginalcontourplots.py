...
class PlotMarginalModes(object):
	...
	def plot_conditional(self, dim1 , dim2 = None, 
		...
		only_interpolate = False, use_log_values = False,
		             grid_points = 40,rank =0, marginalization_type='sum',fout = "r1r2.pdf" 
	):
		"""
        ...
        #add data for observation using the same structure, use directly the prediction file
		...
		if dim2 is not None:
			plt.contourf(grid_x, grid_y, grid_z, levels[::-1], cmap = 'YlGnBu',extend = 'max')
		...
		if with_points and dim2 is not None:
			plt.scatter(dim1_column, dim2_column, marker='+', color='black', s=100, alpha=0.4)
		# add ellipses
		i = 0
		for mode in modes:
...
import json
#json.dump(pnames, open('%s/GUTFITparams.jon'%args[0], 'w'))
#json.dump(pnames, open('%s/params.json'%args[0],'w'))

pnames = json.load(open('%s/GUTFITparams.json'%args[0]))
NP = len(pnames)
print("Analyzing output...")
a = Analyzer(n_params = NP, outputfiles_basename = '%s/GUTFIT'%args[0])
print("Plotting...")

P = PlotMarginalModes(analyser = a)

P.plot_conditional(dim1 = 4, dim2 = 5, with_ellipses = True, with_points = False, grid_points = 400,fout = "r1r2.pdf")
