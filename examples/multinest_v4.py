...
def measure(D, T): 
    ...
    DS =[   abs(np.mean([t[0] for t in T]) - Data[0]),
	    abs(np.mean([t[1] for t in T]) - Data[1]),
    	    abs(np.mean([t[2] for t in T]) - Data[2]),
    	    abs(np.mean([t[3] for t in T]) - Data[3]),
    	    abs(np.mean([t[4] for t in T]) - Data[4]),
    	    abs(np.mean([t[5] for t in T]) - Data[5]),
    	    abs(np.mean([t[6] for t in T]) - Data[6]),
    	    abs(np.mean([t[7] for t in T]) - Data[7])
    ]
    #print(abs(np.mean([t[4] for t in T])))
    print("Benchmark predictions")
    print([np.mean([t[3] for t in T]), np.mean([t[4] for t in T]),np.mean([t[5] for t in T]),np.mean([t[6] for t in T]),np.mean([t[7] for t in T])])
    print("Data")
    print(Data[3:])
    #print(Data[4])
    from functools import reduce
    return sum([(d*d)/(i*i) for d,i in zip(DS, Uncertainties)]) # works technically
