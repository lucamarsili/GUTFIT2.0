...
def plotting(D, T, measure, fout): #Plot the observable distribution for the best fit point
    ...
    for i in range(8):
                ...
                axes[i].hist(theos, bins=10)
                axes[i].hist(values, bins=10)
...
def measure(D, T): #Current measure use only DS and DT, similar to chi square
    OL = [
            overlap([d[0] for d in D], [t[0] for t in T]),
            overlap([d[1] for d in D], [t[1] for t in T]),
            overlap([d[2] for d in D], [t[2] for t in T]),
            overlap([d[3] for d in D], [t[3] for t in T]),
            overlap([d[4] for d in D], [t[4] for t in T]),
            overlap([d[5] for d in D], [t[5] for t in T]),
            overlap([d[6] for d in D], [t[5] for t in T]),
            overlap([d[7] for d in D], [t[5] for t in T])
            ]

    DS = [
            abs(np.mean([d[0] for d in D]) - np.mean([t[0] for t in T])),
            abs(np.mean([d[1] for d in D]) - np.mean([t[1] for t in T])),
            abs(np.mean([d[2] for d in D]) - np.mean([t[2] for t in T])),
            abs(np.mean([d[3] for d in D]) - np.mean([t[3] for t in T])),
            abs(np.mean([d[4] for d in D]) - np.mean([t[4] for t in T])),
            abs(np.mean([d[5] for d in D]) - np.mean([t[5] for t in T])),
            abs(np.mean([d[6] for d in D]) - np.mean([t[6] for t in T])),
            abs(np.mean([d[7] for d in D]) - np.mean([t[7] for t in T]))
            ]
    DT = [ abs(np.amax([d[0] for d in D])-np.amin([d[0] for d in D])),
           abs(np.amax([d[1] for d in D])-np.amin([d[1] for d in D])),
           abs(np.amax([d[2] for d in D])-np.amin([d[2] for d in D])),
           abs(np.amax([d[3] for d in D])-np.amin([d[3] for d in D])),
           abs(np.amax([d[4] for d in D])-np.amin([d[4] for d in D])),
           abs(np.amax([d[5] for d in D])-np.amin([d[5] for d in D])),
           abs(np.amax([d[6] for d in D])-np.amin([d[6] for d in D])),
           abs(np.amax([d[7] for d in D])-np.amin([d[7] for d in D])),
           ]



    from functools import reduce
    return sum([(d*d)/(i*i) for d,i in zip(DS,DT)]) # works technically
